...
$SCRIPT_NAME = "MailContact-DELETE"
$OPERATION_NAME = "deleteOp"
$OBJECT_CLASS_NAME = "MailContact"

try {
    Log "$SCRIPT_NAME Disabling $($OBJECT_CLASS_NAME): $guid" "INFO" "$OPERATION_NAME"
    Disable-MailContact -Identity $guid -DomainController $DOMAIN_CONTROLLER -Confirm:$false -ErrorAction Stop -WarningAction SilentlyContinue
    ...
    try {
        Get-MailContact -Identity $guid -DomainController $DOMAIN_CONTROLLER -ErrorAction Stop -WarningAction SilentlyContinue
    }
    catch {
