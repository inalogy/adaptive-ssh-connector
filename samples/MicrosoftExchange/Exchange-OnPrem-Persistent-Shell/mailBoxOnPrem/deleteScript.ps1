...
$SCRIPT_NAME = "Mailbox-DELETE"
$OPERATION_NAME = "deleteOp"
$OBJECT_CLASS_NAME = "Mailbox"

try {
    Log "$SCRIPT_NAME Disabling $($OBJECT_CLASS_NAME): $guid" "INFO" "$OPERATION_NAME"
    Disable-Mailbox -Identity $guid -DomainController $DOMAIN_CONTROLLER -Confirm:$false -ErrorAction Stop -WarningAction SilentlyContinue
    ...
    try {
        Get-Mailbox -Identity $guid -DomainController $DOMAIN_CONTROLLER -ErrorAction Stop -WarningAction SilentlyContinue
    }
    catch {
